...
import { X, Share2, Coins, Calendar, User as UserIcon, ExternalLink, ShoppingCart, Copy, Check, TrendingUp, ArrowUpRight, Bug, Crown, Zap } from 'lucide-react';
import { 
  NFT, 
  Collection, 
  FractionalNFTInfo, 
...
export default function NFTDetailModal({ nft, collection, userAddress, onClose }: NFTDetailModalProps) {
  ...
  const loadFractionalData = async () => {
    ...
    setIsLoading(true);
    try {
      // Load extended NFT info (including creator)
      ...
      if (userAddress) {
        ...
        // If no user, show total available shares
      ...
      // Calculate max buy amount based on available shares and user balance
  ...
  const handleBuyShares = async () => {
    ...
    try {
      ...
      // Reset buy amount and reload data
      setBuyAmount(1);
  ...
  const isBuyingAllShares = fractionalInfo && buyAmount === fractionalInfo.totalShares;
              ...
              <div>
                ...
                <div className="space-y-2 sm:space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-white font-medium text-sm sm:text-base">#{nft.tokenId}</span>
                  </div>
                  <div className="flex justify-between items-center">
                  ...
                  {fractionalInfo && (
                    <>
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-blue-300 font-medium text-sm sm:text-base flex items-center">
                          <UserIcon className="w-3 h-3 mr-1" />
                          {formatAddress(fractionalInfo.creator || '')}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-400 text-sm sm:text-base">Current Owner</span>
                        <span className="text-green-300 font-medium text-sm sm:text-base flex items-center">
                      ...
                      {/* All shares status */}
                      {allSharesWithOwner && (
                        <div className="bg-blue-900/30 border border-blue-500/50 rounded-lg p-2 mt-2">
                          <div className="text-xs text-blue-300 flex items-center">
                            <Crown className="w-3 h-3 mr-1" />
                            Owner holds all {fractionalInfo.totalShares} shares - ready for full purchase
                          </div>
                        </div>
                      )}
                    </>
                ...
                <div className="bg-purple-900/20 rounded-lg p-4 sm:p-6 border border-purple-500/30">
                  ...
                  <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-white font-medium text-sm sm:text-base">{fractionalInfo.totalShares}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-400 text-sm sm:text-base">Price per Share</span>
                      <span className="text-white font-medium text-sm sm:text-base">{fractionalInfo.sharePrice} ETH</span>
                    </div>
                    {userAddress && userSharePercentage > 0 && (
                      <div className="flex justify-between items-center">
                        <span className="text-gray-400 text-sm sm:text-base">Your Ownership</span>
                        <span className="text-green-400 font-medium text-sm sm:text-base">{userSharePercentage}%</span>
                      </div>
                    )}
                  </div>
                  ...
                  {userAddress ? (
                    <div className="border-t border-purple-500/30 pt-4">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-base sm:text-lg font-medium text-white">Buy Shares</h4>
                      ...
                      {hasAvailableShares ? (
                        <>
                          <div className="mb-3 p-2 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                            <div className="text-xs sm:text-sm text-blue-300">
                              <span className="font-medium">{availableShares}</span> shares available for purchase
                              {allSharesWithOwner && (
                          ...
                          <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 mb-3">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <span className={`font-medium text-sm sm:text-base ${canAfford ? 'text-white' : 'text-red-400'}`}>
                          ...
                          {!canAfford && (
                            <div className="mb-3 p-2 bg-red-900/20 border border-red-500/30 rounded-lg">
                              <div className="text-xs sm:text-sm text-red-300">
                                Insufficient balance. You need {(totalCost - parseFloat(userBalance)).toFixed(4)} more ETH.
                              </div>
                            </div>
                          )}
                        ...
                        <div className="text-center py-4">
                          <div className="text-gray-400 text-sm mb-2">No shares available for purchase</div>
                    ...
                    <div className="border-t border-purple-500/30 pt-4">
                      <div className="text-center py-4">
                        <div className="text-gray-400 text-sm mb-2">Connect your wallet to buy shares</div>
                  ...
                  <div className="border-t border-purple-500/30 pt-4 mt-4">
                    ...
                    >
                      <span className="font-medium text-sm sm:text-base">Shareholders ({shareHolders.length})</span>
                      <span className={`transform transition-transform text-sm ${showShareholders ? 'rotate-180' : ''}`}>
                        ▼
                      </span>
                    </button>
                    ...
                    {showShareholders && (
                      <div className="mt-3 space-y-2 max-h-32 sm:max-h-40 overflow-y-auto">
                        {shareHolders.map((holder, index) => (
                          <div key={index} className="flex items-center justify-between text-xs sm:text-sm bg-gray-800/30 rounded p-2">
                            <span className="text-gray-400 font-mono truncate flex-1 mr-2">{formatAddress(holder.holder)}</span>
