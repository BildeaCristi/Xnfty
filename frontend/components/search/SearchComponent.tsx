...
import { Search, X, Filter, Loader2, Image, Folder } from 'lucide-react';
import { useRouter } from 'next/navigation';
import type { Collection, SearchResult, SearchFilters } from '@/types';
import { SearchEngine, highlightSearchTerm, truncateText } from '@/utils/search';
...
export default function SearchComponent({ 
  ...
  // Update search engine when collections change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const searchTimeout = setTimeout(async () => {
      ...
      try {
        ...
        setShowResults(true);
        console.log('✅ Dropdown should show. showResults:', true, 'results length:', searchResults.length);
      } catch (error) {
  ...
  const handleResultClick = (result: SearchResult) => {
    ...
    if (result.type === 'collection') {
      ...
      // Navigate to collection page, the NFT modal can be opened there
      router.push(`/collections/${result.collectionId}?nft=${result.tokenId}`);
    }
  };
  ...
  return (
    <div ref={searchRef} className={`relative ${className}`}>
      ...
      {showResults && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-gray-900/98 backdrop-blur-md border border-gray-700 rounded-lg shadow-2xl z-[100]">
          {/* Filter Options */}
