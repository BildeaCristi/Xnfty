...
import { X, Upload, Loader2, Plus, Trash2, CheckCircle, AlertCircle } from 'lucide-react';
import { uploadFileToIPFS, uploadCollectionMetadata, createCollectionMetadata, uploadNFTMetadata, createNFTMetadata, uploadJSONToIPFS } from '@/utils/ipfs';
import { createCollection, mintNFT, fractionalizeNFT, getNFTFactoryContract, getSigner } from '@/utils/blockchain';
        ...
        <div className="px-6 py-4 bg-gray-800/50">
          <div className="flex items-center space-x-4">
            <div className={`flex items-center space-x-2 ${step >= 1 ? 'text-blue-400' : 'text-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step >= 1 ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-400'}`}>
                1
              </div>
            ...
            <div className={`flex items-center space-x-2 ${step >= 2 ? 'text-blue-400' : 'text-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step >= 2 ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-400'}`}>
                2
              </div>
            ...
            <div className={`flex items-center space-x-2 ${step >= 3 ? 'text-blue-400' : 'text-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step >= 3 ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-400'}`}>
                3
              </div>
              ...
              <>
                ...
                {nfts.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {nfts.map((nft, index) => (
                      <div key={nft.id} className="border border-gray-700 rounded-lg p-6 bg-gray-800/50">
                            ...
                            <div>
                              ...
                              <div className="flex items-center space-x-4">
                                {nft.imagePreview ? (
                                  ...
                                  <div className="w-20 h-20 border-2 border-dashed border-gray-600 rounded-lg flex items-center justify-center bg-gray-800">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {deploymentSteps.map((step) => (
                    <div key={step.id} className="flex items-center space-x-4 p-4 bg-gray-800/50 rounded-lg border border-gray-700">
