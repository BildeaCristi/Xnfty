...
import { UserNFTShare } from '@/utils/blockchain';
        ...
        <div className="mb-3">
          ...
          <div className="space-y-1">
            {fullyOwnedNFTs.map((nft, idx) => (
              <div key={idx} className="text-xs text-green-200 flex items-center justify-between bg-green-900/20 rounded p-2">
        ...
        <div>
          ...
          <div className="space-y-1">
            {partiallyOwnedNFTs.map((nft, idx) => (
              <div key={idx} className="text-xs text-purple-200 flex items-center justify-between bg-purple-900/20 rounded p-2">
                <span>NFT #{nft.tokenId} in Collection #{nft.collectionId}</span>
                <span className="font-medium">{nft.sharePercentage}% ({nft.userShares}/{nft.totalShares} shares)</span>
              </div>
            ))}
          </div>
        </div>
