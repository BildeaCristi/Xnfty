...
export default function EnhancedNFTFrame({
  nft,
  position,
  rotation = [0, 0, 0],
  scale = 1,
  ...
  // Floating animation for special NFTs
  const isRare = nft.tokenId < 100; // First 100 NFTs are considered rare
  ...
  const frameContent = (
    ...
    >
      ...
      {/* Glow effect for rare NFTs */}
      {isRare && (
        <animated.mesh position={[0, 0, -0.1]} scale={1.1}>
          <planeGeometry args={[
            frameSize.width + 0.4,
            frameSize.height + 0.4
          ]} />
          <animated.meshBasicMaterial
            transparent
            opacity={springs.glowIntensity}
            color={nft.tokenId < 10 ? '#FFD700' : '#B87FFF'}
